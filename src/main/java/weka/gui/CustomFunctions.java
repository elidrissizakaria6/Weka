...
public class CustomFunctions
   ...
   */
  public CustomFunctions() throws Exception {
    setTitle(Messages.getInstance().getString("CustomFunctions_SetTitle_Text"));
    setLayout(new BorderLayout());
    add(new CustomFunctionsPanel());
    pack();
    setSize(600, 500);
    setVisible(true);
  }
  ...
  public static void main(String[] args) {
    ...
    try {
      final CustomFunctions frame = new CustomFunctions();
      frame.addWindowListener(new WindowAdapter() {
	public void windowClosing(WindowEvent param1) {
	  System.err.println(Messages.getInstance().getString("CustumFunctions_Main_Error_Text"));
	  frame.dispose();
	}
      });
